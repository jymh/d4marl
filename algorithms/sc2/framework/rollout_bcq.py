...
def sample(model, raw_model, state, device, available_actions=None, sample=False):
    ...
    softmax_actor_q = F.softmax(actor_q, dim=-1)
    candidate_a = (softmax_actor_q / softmax_actor_q.max(-1, keepdim=True).values
                   > raw_model.threshold).float().to(device)

    # if available_actions is not None:
    #     candidate_a[available_actions == 0] = 0.

    next_action = (candidate_a * softmax_actor_q +
                   (torch.ones(1, 1, 1).to(device) - candidate_a) * -1e8).argmax(-1, keepdim=False)
